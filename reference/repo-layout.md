...
<project>/
  ...
  output/            what the macros write: output/<family>/<SKU>/<rev>/ — gitignored,
                     reproducible from config (see output-convention.md)
  exports/           CURATED + COMMITTED: vendor bundles, latest STEP per part (qty in names)
  tools/             non-geometry helpers (render_views.FCMacro, exporters)
  README.md          one-line regenerate command per macro
  .gitignore         output/  *.FCStd1  *.FCBak  .DS_Store  *.FCStd.*
```

Rules
- `output/` is disposable and gitignored. Wrong output ⇒ fix macro/config,
  re-run. Each `<rev>` carries a `cad-manifest.json` pinning the config sha.
- `exports/` is the committed, curated subset (vendor bundles, latest STEP per
  part) so collaborators without FreeCAD still get files; refresh by copying
  from the current rev, never by editing.
